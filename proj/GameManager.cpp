...
void GameManager::specialKeyPressed(unsigned char key) {
	...
	case GLUT_KEY_UP:
		car->setSpeed(
				car->getSpeed()->getX()
						+ ACCELERATION_FORWARD * car->getDirecao()->getX()
								* _delta_t,
				car->getSpeed()->getY()
						+ ACCELERATION_FORWARD * car->getDirecao()->getY()
...
void GameManager::specialKeyUP(unsigned char key) {
	...
	case GLUT_KEY_UP:
		car->setSpeed(car->getSpeed()->getX() * exp(-_delta_t),
				car->getSpeed()->getY() * exp(-_delta_t),
				car->getSpeed()->getZ());
		break; //up key
